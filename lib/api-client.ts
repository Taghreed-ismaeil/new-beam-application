...
// AuthProvider registers its own logout() here on mount. A 401 means the stored
// session is no longer valid (expired token, or the account it points to is
// gone) — clearing it lets AuthGate's existing redirect-to-login logic take
// over, instead of every screen having to know how to recover from this itself.
...
export async function apiRequest<T = any>(
  ...
  if (!res.ok) {
    ...
    const message =
      data?.message || data?.error || `request_failed_${res.status}`;
...
export function apiRequestCached<T = any>(
  path: string,
  onData: (data: T) => void,
): void {
